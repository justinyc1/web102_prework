...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++) {
        ...
        newDiv.innerHTML = `
            <div>
                <div>
                    <img className="game-img" src=${games[i].img} width="300" height="150" />
                </div>
                <div>
                    <h3>${games[i].name}</h3>
...
function showAllGames() {
    ...
    let allGames = GAMES_JSON.filter(() => true);
...
has been raised for ${GAMES_JSON.length} game${GAMES_JSON.length === 1 ? "" : "s"}. 
Currently, ${unfundedGames.length} game${unfundedGames.length === 1 ? "" : "s"} remain${unfundedGames.length === 1 ? "s" : ""} unfunded. 
We need your help to fund these amazing games!`

// create a new DOM element containing the template string and append it to the description container
const newDescription = document.createElement("div");
newDescription.classList.add("descriptionContainer");
newDescription.innerHTML = `<p>${displayStr}</p>`;
